...
def convertir_unites(quantite, unite_source, unite_cible="g"):
    conversions = {"g": 1, "kg": 1000, "cl": 10, "ml": 1, "l": 1000, "unites": 1}
    unite_source = unite_source.lower()
    unite_cible = unite_cible.lower()
    # Ajuster l'unité cible par défaut selon le type (masse ou volume)
    if unite_source in ["ml", "cl", "l"] and unite_cible == "g":
        unite_cible = "ml"
    quantite_base = quantite * conversions.get(unite_source, 1)
    return quantite_base / conversions.get(unite_cible, 1)
    ...
    try:
        ...
        for ing_recette in ingredients_recette:
            ...
            if ing_inventaire:
                ...
                quantite_manquante = convertir_unites(quantite_manquante_base, "g", unite_recette) if quantite_manquante_base > 0 else 0
